...
RSpec.describe "Drivers", type: :request do
  let!(:users) { create_list(:user, 5, role:'driver', user_type:'driver')}
  ...
  describe "GET drivers" do 
    before { get "/admins/drivers", headers: headers}
    it 'returns users' do 
  ...
  describe "GET /admins/drivers/:id" do
    before { get "/admins/drivers/#{user_id}", headers: headers}
    ...
    describe "PUT /admins/drivers/:id" do
      context "when request is valid" do
        before{ put "/admins/drivers/#{user_id}", params:{role: "admin" }.to_json, headers: headers }
    ...
    describe "PUT /admins/drivers/:token/update_profile" do
      let(:params){{profile_picture: Faker::Avatar.image, address:Faker::Address.street_address, resident_state: 'Bayelsa' }.to_json}
      before { put "/admins/drivers/#{user_id}/update_profile", params: params, headers:headers }
